module DiskMon
    ...
    class InstanceRepository
            ...
            def find_instances_by_name(name)
                ...
                result.reservations.map {|reservation| reservation.instances}.flatten
            end

            def create_direct_access_instance(name)
                find_instances_by_name(name).map do |instance|
                    DirectAccessInstance.new(instance.instance_id, instance.key_name, instance.public_dns_name, name)
                end
            end
            ...
            def create_private_access_instance(name)
                find_instances_by_name(name).map do |instance|
                    PrivateAccessInstance.new(instance.instance_id, instance.key_name, instance.private_dns_name, name)
                end
            end
            ...
            def create_proxied_instance(bastion_name, target_name)
                ...
                create_private_access_instance(target_name).map{|instance| ProxiedInstance.new(bastion, instance)}
            end
